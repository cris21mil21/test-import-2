...
(function ($, Drupal) {
  ...
  Drupal.ImageWidgetCropType = function (instance, element) {
    ...
    this.$chiudi = this.$wrapper.find(this.selectors.chiudi);
  ...
  Drupal.ImageWidgetCropType.prototype.init = function () {
    ...
    this.$chiudi
      .on('click.iwc', this.chiudi.bind(this));
   ...
   *   The event object.
   */
  Drupal.ImageWidgetCropType.prototype.chiudi = function (e) {
    if (!this.cropper) {
      return;
    }
    ...
    const target = $('.image-data__crop-wrapper')
    target[0].removeAttribute("open")
